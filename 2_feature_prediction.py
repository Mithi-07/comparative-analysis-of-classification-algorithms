...
plot_decision_regions(X_test.values, y_test.values, clf=knn_classifier, legend=2)
plt.xlabel('Glucose')
plt.ylabel('BMI')
plt.title('K-NN Classifier Decision Boundary')
plt.show()

# Plot decision regions for Logistic Regression classifier
plt.figure(figsize=(8, 6))
plot_decision_regions(X_test.values, y_test.values, clf=lr_classifier, legend=2)
plt.xlabel('Glucose')
plt.ylabel('BMI')
